import { get, ui, game, _status, lib } from "/noname.js";
...
const skills = {
  ...
  new_standard_tuxi: {
    ...
    async cost(event, trigger, player) {
      ...
      const result = await player.chooseToDiscard(
        ...
        const targets = game.filterPlayer(
          (current) => player2 != current && current.countGainableCards(player2, "h") && get.effect(current, { name: "shunshou_copy2" }, player2, player2) > 0
        );
    ...
    async content(event, trigger, player) {
      ...
      player.when({ global: `${phaseEvent.name}After` }).filter((endingPhaseEvent) => endingPhaseEvent == phaseEvent).step(async (event2, trigger2, player2) => {
        delete player2.storage.new_standard_tuxi_phaseEvent;
      });
      ...
      const { bool, targets } = await player.chooseTarget(
        ...
        (card, player2, target) => {
          return player2 != target && target.countGainableCards(player2, "h");
        },
        [1, num],
        true
      ).set("ai", (target) => {
  ...
  new_standard_xunxun: {
    ...
    async content(event, trigger, player) {
      ...
      const result = await player.chooseToMove("恂恂：将两张牌置于牌堆顶（靠左的牌更靠上）", true).set("list", [["牌堆顶", cards], ["牌堆底"]]).set("filterMove", function(from, to, moved) {
        ...
        const cards2 = list[0][1].slice(0).sort(function(a, b) {
          return get.value(b) - get.value(a);
        });
        return [cards2, cards2.splice(2)];
      }).forResult();
  ...
  new_standard_wangxi: {
    ...
    async content(event, trigger, player) {
      ...
      await game.cardsGotoOrdering(cards);
      const result = await player.chooseCardButton(`忘隙：选择交给${get.translation(target)}的一张牌`, cards, true).set("ai", (button) => {
        const player2 = get.player();
        const target2 = get.event().target;
        const att = get.attitude(player2, target2);
        const value = get.value(button.link, target2);
  ...
  new_standard_longdan: {
    ...
    subSkill: {
      cover: {
        ...
        priority: 10,
        filter(event, player) {
          return event.card.name == "sha" && event.player != player && event.target != player && !event.targets.includes(player) && get.distance(player, event.target) <= 1 && !player.getStorage("new_standard_longdan_record").includes(event.target) && lib.filter.targetEnabled(event.card, event.player, player);
        },
        async cost(event, trigger, player) {
          const result = await player.chooseBool(get.prompt("new_standard_longdan", trigger.target), "代替其成为此【杀】的目标").set("ai", () => {
            const trigger2 = get.event().getTrigger();
            const player2 = get.player();
            return get.effect(player2, trigger2.card, trigger2.player, player2) >= get.effect(trigger2.target, trigger2.card, trigger2.player, player2);
          }).forResult();
          event.result = result;
        },
        async content(event, trigger, player) {
        ...
        async content(event, trigger, player) {
          if (!player.getStorage("new_standard_longdan_record").length) {
            player.when({ global: "phaseAfter" }).step(async (event2, trigger2, player2) => {
              delete player2.storage.new_standard_longdan_record;
            });
          }
  ...
  new_standard_yajiao: {
    ...
    async content(event, trigger, player) {
      ...
      await player.showCards(cards, get.translation(player) + "发动了【涯角】", true).set("type", get.type2(trigger.getParent().card)).set("clearArena", false).set("removeHighlight", false).set("callback", async (yajiaoEvent) => {
        const [card] = yajiaoEvent.cards;
        const evt = yajiaoEvent.getParent();
        const { type, videoId, highlightRemove } = evt;
        if (get.type2(card) == type) {
          const result = await player.chooseTarget("涯角：选择获得此牌的角色").set("ai", (target) => {
            const player2 = get.player();
          ...
          const result = await player.chooseTarget("涯角：是否弃置攻击范围内包含你的一名角色区域内的一张牌？", (card2, player2, target) => {
            return target.inRange(player2) && target.countDiscardableCards(player2, "hej") > 0;
          }).set("ai", (target) => {
            const player2 = get.player();
  ...
  new_standard_tieji: {
    ...
    async content(event, trigger, player) {
      ...
      const discardResult = await target.chooseToDiscard("请弃置一张" + get.translation(suit) + "牌，否则不能使用【闪】响应此【杀】", "he", (card) => {
        ...
        return 8 - get.value(card);
      }).set("shanNum", shanNum).set("suit", suit).forResult();
  ...
  new_standard_qingguo: {
    mod: {
      aiValue(player, card, num) {
        ...
        const cards = player.getCards("hs", (card2) => get.name(card2) == "shan" || get.color(card2) == "black");
        cards.sort((a, b) => {
  ...
  new_standard_zhonghu: {
    ...
    async content(event, trigger, player) {
      ...
      if (controls.length > 1) {
        const result = await target.chooseControl(controls).set("choiceList", choiceList).set("prompt", "冢虎：请选择一项").set("ai", () => {
          const player2 = get.event().player;
          const source = get.event().source;
          const target2 = get.event().target;
          const targetShaCards2 = get.event().targetShaCards;
          const judgeEffect = targetShaCards2.reduce((sum, card) => {
            const name = get.color(card, target2) == "red" ? "lebu" : "bingliang";
            return Math.min(sum, get.effect(target2, { name }, source, player2));
          }, 0);
          const duelEffect = get.effect(target2, { name: "sha" }, source, player2) + get.effect(source, { name: "sha" }, target2, player2);
          return judgeEffect < duelEffect ? "选项一" : "选项二";
        }).set("source", player).set("target", target).set("targetShaCards", targetShaCards).forResult();
      ...
      if (control == "选项一") {
        ...
        while (player.isIn() && target.isIn() && failCount < 2) {
          const result = await current.chooseToUse({
            ...
            filterCard(card, player2, event2) {
              return get.name(card, player2) == "sha" && lib.filter.cardEnabled(card, player2, event2);
            },
            filterTarget(card, player2, target2) {
              if (target2 != get.event().opponent || !card || !get.info(card)) {
                return false;
              }
              return player2.canUse(card, target2, false, false);
            }
          }).set("opponent", opponent).forResult();
        ...
        const result = await player.chooseCardButton(`冢虎：选择${get.translation(target)}的一张【杀】`, targetShaCards, true).set("ai", (button) => {
          const player2 = get.player();
          const target2 = get.event().target;
          const card = button.link;
          const name = get.color(card, target2) == "red" ? "lebu" : "bingliang";
          return get.effect(target2, { name }, player2, player2);
        }).set("target", target).forResult();
  ...
  new_standard_qixi: {
    ...
    subSkill: {
      visible: {
        ...
        async cost(event, trigger, player) {
          const result = await player.chooseBool(get.prompt(event.skill, trigger.target), "令其手牌在此次【过河拆桥】结算中可见").set("choice", get.attitude(player, trigger.target) <= 0).forResult();
          event.result = result;
        },
        async content(event, trigger, player) {
  ...
  new_standard_fenwei: {
    ...
    filter(event, player) {
      return event.isFirstTarget && get.type(event.card) == "trick" && event.targets?.length > 1 && event.targets.some((target) => !event.getParent().excluded.includes(target)) && !player.storage.new_standard_fenwei;
    },
    async cost(event, trigger, player) {
      const targets = trigger.targets.filter((target) => !trigger.getParent().excluded.includes(target));
      const result = await player.chooseTarget(get.prompt(event.skill), "令此牌对任意名目标角色无效", [1, targets.length], (card, player2, target) => {
        return get.event().targets.includes(target);
      }).set("targets", targets).set("ai", (target) => {
        const trigger2 = get.event().getTrigger();
        return -get.effect(target, trigger2.card, trigger2.player, get.player());
      }).forResult();
      event.result = result;
    },
    async content(event, trigger, player) {
  ...
  new_standard_xiaoguo: {
    ...
    async cost(event, trigger, player) {
      event.result = await player.chooseToDiscard("he", get.prompt(event.skill, trigger.player), "弃置一张基本牌，令其弃置一张装备牌并令你摸一张牌，否则其受到你造成的1点伤害", (card) => {
    ...
    logTarget: "player",
    async content(event, trigger, player) {
      const result = await trigger.player.chooseToDiscard("he", "骁果：弃置一张装备牌并令" + get.translation(player) + "摸一张牌，或受到1点伤害", (card) => {
        return get.type(card) == "equip";
      }).set("ai", (card) => {
        const player2 = get.player();
        const source = get.event().source;
        if (get.damageEffect(player2, source, player2) >= 0) {
          return 0;
        }
  ...
  new_standard_rende: {
    ...
    async content(event, trigger, player) {
      ...
      if (!oldGiven) {
        player.when({ player: "phaseUseEnd" }).step(async (event2, trigger2, player2) => {
          player2.clearMark("new_standard_rende", false);
        });
      }
      ...
      if (oldGiven < 2 && oldGiven + event.cards.length >= 2) {
        ...
        const result = await player.chooseButton(["仁德：是否视为使用一张基本牌？", [list, "vcard"]]).set("ai", (button) => get.player().getUseValue({ name: button.link[2], nature: button.link[3], isCard: true })).forResult();
        if (result?.bool && result.links?.length) {
  ...
  new_standard_guanxing: {
    ...
    async content(event, trigger, player) {
      ...
      await game.cardsGotoOrdering(cards);
      const result = await player.chooseToMove("观星：点击将牌移动到牌堆顶或牌堆底", true).set("list", [["牌堆顶", cards], ["牌堆底"]]).set("processAI", (list) => {
        const cards2 = list[0][1];
        const player2 = _status.event.player;
        const target = _status.event.getTrigger().name == "phaseZhunbei" ? player2 : player2.next;
        const att = get.sgn(get.attitude(player2, target));
        const top2 = [];
        const judges = target.getCards("j");
        let stopped = false;
        if (player2 != target || !target.hasWuxie()) {
        ...
        if (!stopped) {
          cards2.sort((a, b) => (get.value(b, player2) - get.value(a, player2)) * att);
          while (cards2.length) {
            if (get.value(cards2[0], player2) <= 5 == att > 0) {
              break;
            }
            top2.unshift(cards2.shift());
          }
      ...
      const bottom = result.moved[1];
      top.reverse();
      await game.cardsGotoPile(top.concat(bottom), ["top_cards", top], (event2, card) => event2.top_cards.includes(card) ? ui.cardPile.firstChild : null);
